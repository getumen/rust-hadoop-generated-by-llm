...
- **認証 (SigV4/STS)**: AWS Signature Version 4 によるリクエストの認証に加え、外部 IdP (OIDC) と連携した一時的な認証情報の発行 (AssumeRoleWithWebIdentity) をサポート。
- **IAM ポリシー評価**: 役割 (Role) ベースのアクセスコントロールをサポート。
- **監査ログ (Audit Logging)**: セキュリティイベント（認証・認可）をRocksDBに記録し、保持期間管理が可能。
...
| `S3_ACCESS_KEY`             | (空)        | サーバー側で受け入れるアクセスキー（静的な IAM クレデンシャル）。 |
| `S3_SECRET_KEY`             | (空)        | サーバー側で保持するシークレットキー。                            |
| `OIDC_ISSUER_URL`           | (空)        | OIDC プロバイダの Issuer URL。                                    |
| `OIDC_CLIENT_ID`            | (空)        | OIDC クライアント ID (Audience 検証用)。                          |
| `STS_SIGNING_KEY`           | (空)        | STS セッショントークンの暗号化用共通鍵 (32 bytes)。               |
| `IAM_CONFIG_PATH`           | (空)        | IAM ロールとポリシーを定義する `iam_config.json` へのパス。       |
| `AUDIT_LOG_ENABLED`         | `true`      | 監査ログを有効化するかどうか。                                    |
| `AUDIT_LOG_DIR`             | `/tmp/s3_audit_log` | 監査ログを保存する RocksDB のディレクトリパス。          |
| `AUDIT_LOG_RETENTION_DAYS`  | `30`        | 監査ログの保持日数。                                              |
| `AUDIT_LOG_BATCH_SIZE`      | `100`       | 監査ログの書き込みバッチサイズ。                                  |

### 監査ログの閲覧 (audit_reader)

保存された監査ログは、付属の `audit_reader` ツールを使用して CLI から閲覧・フィルタリングできます。

```bash
# 全てのログを表示
./target/debug/audit_reader /tmp/s3_audit_log

# 特定のユーザー、アクション、時間範囲でフィルタリング
./target/debug/audit_reader /tmp/s3_audit_log --user my-user --action s3:GetObject --start 2024-03-19T00:00:00Z

# JSON 形式で出力（集計や他ツールとの連携に便利）
./target/debug/audit_reader /tmp/s3_audit_log --json
```
...
- **STS**: `AssumeRoleWithWebIdentity` による一時クレデンシャル発行

## 制限事項と今後の改善

- **認証**: SigV4 (Core) および OIDC/STS (Phase 1-3) が実装済みです。静的な `iam_config.json` によるロールベースの認可をサポートしています。
- **CopyObject**: 現在、サーバー側で一度ファイルを一時的に読み込み、再書き込みするナイーブな実装となっています。将来的にはメタデータ操作のみで完結する最適化を予定しています。
- **Presigned URLs**: 現在未実装です。将来のフェーズで実装予定です。
